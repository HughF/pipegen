...
#   -> dist/pipegen-<version>-x86_64.AppImage
#
...
VERSION="$(sed -n 's/.*PIPEGEN_VERSION "\([^"]*\)".*/\1/p' "$ROOT/src/pg_version.h")"
OUT="$ROOT/dist/pipegen-$VERSION-x86_64.AppImage"
...
APPDIR="$ROOT/dist/pipegen.AppDir"
rm -rf "$APPDIR"
mkdir -p "$APPDIR/usr/bin" "$APPDIR/usr/lib" \
         "$APPDIR/usr/share/applications" \
         "$APPDIR/usr/share/icons/hicolor/256x256/apps" \
         "$APPDIR/usr/share/pipegen" \
         "$APPDIR/usr/share/doc/pipegen"

cp "$ROOT/pipegen"                "$APPDIR/usr/bin/"
cp "$HERE/AppRun"                 "$APPDIR/AppRun"
cp "$HERE/pipegen.desktop"        "$APPDIR/pipegen.desktop"
cp "$HERE/pipegen.desktop"        "$APPDIR/usr/share/applications/"
cp "$HERE/pipegen.png"            "$APPDIR/pipegen.png"
cp "$HERE/pipegen.png"            "$APPDIR/usr/share/icons/hicolor/256x256/apps/"
cp "$ROOT/LICENSE"                "$APPDIR/usr/share/doc/pipegen/"
cp "$ROOT/docs/HELP.md"           "$APPDIR/usr/share/doc/pipegen/"
...
# and exactly the sort of machine an AppImage gets carried onto. pg_ui.c looks
# here first, relative to the executable, because an AppImage is mounted
# somewhere different every time it runs.
cp "$HERE/font/DejaVuSans.ttf"      "$APPDIR/usr/share/pipegen/"
cp "$HERE/font/DejaVu-LICENSE.txt"  "$APPDIR/usr/share/doc/pipegen/"
